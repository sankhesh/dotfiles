...
for x, y, scale, transform, primary, linked_monitors_info, props in logical_monitors:
    ...
    for linked_monitor_connector, linked_monitor_vendor, linked_monitor_product, linked_monitor_serial in linked_monitors_info:
        ...
        #   x = 0
    updated_logical_monitor_struct = dbus.Struct([dbus.Int32(x), dbus.Int32(y), dbus.Double(
        scale), dbus.UInt32(transform), dbus.Boolean(primary), physical_monitors_config])
...
display_config_interface.ApplyMonitorsConfig(dbus.UInt32(serial), dbus.UInt32(
    method), updated_logical_monitors, properties_to_apply)
